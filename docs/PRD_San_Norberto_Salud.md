...
- **Paso 3 (conectar backend) y Paso 4 (autenticación y RLS)**: iniciados en local (Docker), con alcance mínimo viable:
  ...
  - Conectado a Supabase real (`VITE_USE_SUPABASE=true`): catálogos `sectors`, `clinics`, `diseases`, `age_ranges`, `gender_categories`, `profiles`, y el flujo núcleo `health_records` (RF-08 a RF-16), con Row Level Security activo (`RNF-01`).
