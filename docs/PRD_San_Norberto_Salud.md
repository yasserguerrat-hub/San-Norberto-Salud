...
- **Paso 3 (conectar backend) y Paso 4 (autenticación y RLS)**: iniciados en local (Docker), con alcance mínimo viable:
  ...
  - **Sección 6.2 conectada a Supabase real**: `commune_population`, `sector_population`, `demographic_population` (gestión de población desde la página Población), `risk_thresholds` y `percentage_thresholds` (umbrales configurables desde Configuración, `RF-07`); la validación de superposición de rangos de edad (`RF-06`) opera sobre los datos reales. **Gestión de sectores (RF-05) completada**: la página Sectores ya permite crear, editar y eliminar sectores (antes solo mostraba el catálogo sembrado, sin CRUD) — mismo patrón que Enfermedades/Rangos de edad/Género. El borrado respeta dependencias (clínicas, población, registros) vía las restricciones de llave foránea de Postgres, con mensaje traducido en `createSupabaseRepository` (código `23503`) en vez del error crudo de Postgrest, beneficiando a todos los catálogos.
